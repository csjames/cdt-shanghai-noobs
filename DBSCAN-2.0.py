...
def compute(df, HAPPY_PICKUP_WALKING_DISTANCE, HAPPY_WALKING_DISTANCE, MAX_WAITING_TIME):
    ...
    epsilon = 0.5*(HAPPY_PICKUP_WALKING_DISTANCE+HAPPY_WALKING_DISTANCE) / 6371.0088  # kilometers per radian
    a = 4
...
# exclude labels= -1 which is noise
                ...
                if (j not in used0) & (db1.labels_[j]!=-1) & (db2.labels_[j]!=-1):
                    for k in range(len(dfp.index)-1):
                        if (k not in used0)&(k!=j)&(db1.labels_[k]!=-1)& (db2.labels_[k]!=-1):
...
inputfile = r'C:/Users/FY/Desktop/2013-11-25-40k.csv'
